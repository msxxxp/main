...
struct File: public FsNode {
	...
	File(const WIN32_FIND_DATAW & info, parent_type parent);
...
//	}
	...
	void LoadHashMini() const
	{
		if (!m_hashMini.is_available()) {
			m_hashMini(get_full_path().c_str(), Global::firstBlockHash);
		}
	}

	void LoadHashFull() const
	{
		if (!m_hashFull.is_available()) {
			m_hashFull(get_full_path().c_str());
		}
	}
