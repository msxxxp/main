...
namespace Com {
	...
	WmiObject::WmiObject(const pointer p) :
		Object<IWbemClassObject>(p)
	{
	}

	WmiObject::WmiObject(const Variant & param) :
		Object<IWbemClassObject>(param)
	{
	}
...
///=================================================================================== WmiConnection
	...
	{
		Object<IWbemLocator> wbemLocator;
		...
		::_snwprintf(Namespace, Base::lengthof(Namespace), L"\\\\%s\\root\\%s", srv, namesp);

		CheckCom(wbemLocator->ConnectServer(Namespace, BStr(user), BStr(pass), nullptr, 0, nullptr, nullptr, &m_svc));
		CheckCom(
		    ::CoSetProxyBlanket(m_svc, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, nullptr, RPC_C_AUTHN_LEVEL_PKT_PRIVACY/*RPC_C_AUTHN_LEVEL_DEFAULT*/, RPC_C_IMP_LEVEL_IMPERSONATE, nullptr, EOAC_STATIC_CLOAKING));
...
///=========================================================================================s WmiBase
	...
	WmiBase::WmiBase(const WmiConnection & conn, const BStr & path) :
		m_conn(conn), m_obj(m_conn.get_object(path)), m_path(path)
	{
	}

	WmiBase::WmiBase(const WmiConnection & conn, const WmiObject & obj) :
		m_conn(conn), m_obj(obj), m_path(WmiObject::get_path(obj))
	{
	}
...
///====================================================================================== WMIProcess
	...
	BStr WmiProcess::Path(DWORD id) const
	{
		...
		::_snwprintf(path, Base::lengthof(path), L"Win32_Process.Handle=%d", id);
...
///==================================================================================== WmiProcessor
	...
	{
		wchar_t path[MAX_PATH];
		::_snwprintf(path, Base::lengthof(path), L"Win32_Processor.DeviceID='CPU%d'", id);
...
///======================================================================================= WmiSystem
	...
	{
		wchar_t path[MAX_PATH];
		::_snwprintf(path, Base::lengthof(path), L"Win32_ComputerSystem", name);
