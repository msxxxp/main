...
///======================================================================================= QuotaInfo
	QuotaInfo::QuotaInfo(const Object<IDiskQuotaControl> &ctrl, PCWSTR name) :
		m_name(name)
	{
		CheckApiError(ctrl->FindUserName(name, &m_usr));
	}

	QuotaInfo::QuotaInfo(const Object<IDiskQuotaUser> &usr) :
		m_usr(usr)
	{
	}
	...
	ustring QuotaInfo::get_name(const Object<IDiskQuotaUser> &usr)
	{
		try {
...
///======================================================================================== NetQuota
	...
	DiskQuota::DiskQuota(const ustring &path) :
		m_path(path)
	{
		CheckApiError(::CoCreateInstance(CLSID_DiskQuotaControl, nullptr, CLSCTX_INPROC_SERVER, IID_IDiskQuotaControl, (PVOID* )&m_control));
		CheckApiError(m_control->Initialize(m_path.c_str(), true));
	}

	const Object<IDiskQuotaControl>& DiskQuota::operator->() const
	{
		return m_control;
	}
...
//change limits
	...
	Object<IDiskQuotaUser> DiskQuota::get_user_raw(const ustring & name) const
	{
		Object<IDiskQuotaUser> ret;
		CheckApiError(m_control->FindUserName(name.c_str(), &ret));
		return ret;
	}

	Object<IDiskQuotaUser> DiskQuota::add_raw(const ustring & name)
	{
		Object<IDiskQuotaUser> ret;
	...
	Object<IDiskQuotaUser> DiskQuota::add_raw(const ustring & name, size_t lim, size_t thr)
	{
		Object<IDiskQuotaUser> ret;
		CheckApiError(m_control->AddUserName(name.c_str(), DISKQUOTA_USERNAME_RESOLVE_ASYNC, &ret));
	...
	void DiskQuota::del(const Object<IDiskQuotaUser> & user)
	{
...
///=================================================================================== NetQuotaUsers
	...
	{
		{ // precache
			Object<IEnumDiskQuotaUsers> qenum;
	...
	void DiskQuotaUsers::add(const ustring & name, size_t lim, size_t thr)
	{
		...
		if (it != end()) {
			...
			Object<IDiskQuotaUser> usr(m_nq.add_raw(name, lim, thr));
