...
///==================================================================================== WinScheduler
	...
	private:
		WinScheduler()
		{
			CheckApiError(::CoCreateInstance(CLSID_CTaskScheduler, nullptr, CLSCTX_INPROC_SERVER, IID_ITaskScheduler, (PVOID* )&m_ts));
		}

		Object<ITaskScheduler> m_ts;
	};
...
///====================================================================================== WinTrigger
	...
	WinTrigger::WinTrigger(size_t index, const Object<ITaskTrigger> &trigger) :
		m_trigger(trigger), m_info(new _TASK_TRIGGER), m_index(index)
	{
...
//}
	...
	WinTrigger::WinTrigger(size_t index, const Object<ITask> &task) :
		m_info(new _TASK_TRIGGER), m_index(index)
	{
...
///========================================================================================= WinTask
		...
		m_name(name)
	{
		try {
			...
			if (autocreate && e.code() == (DWORD)HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND)) { //COR_E_FILENOTFOUND
				CheckApiError(WinScheduler::instance()->NewWorkItem(name.c_str(), CLSID_CTask, IID_ITask, (IUnknown** )&m_task));
				return;
			}
			throw;
		}
	...
	WinTask::operator Object<ITask>() const
	{
		return m_task;
	}

	Object<ITask> WinTask::operator->() const
	{
		return m_task;
	}
	...
	{
		Object<ITaskTrigger> pITaskTrigger;
		WORD index;
	...
	{
		Object<IPersistFile> persistFile;
...
//		CheckApiError(m_task->GetTriggerString(i, &str));
			Object<ITaskTrigger> taskTrigger;
