...
namespace Far {
	...
	DialogImpl::DialogImpl(const GUID & dguid, int X1, int Y1, int X2, int Y2, FarDialogItem* Item, int ItemsNumber, DWORD Flags, FARWINDOWPROC DlgProc) :
		m_hndl(nullptr)
	{
	...
	{
		simstd::unique_ptr<DialogImpl> tmp(new DialogImpl(guid, x1, y1, x2, y2, items, count, flags, nullptr));
