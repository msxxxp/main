...
namespace linkage {
	...
	void CAPIHook::ReplaceIATEntryInAllMods(PCSTR pszCalleeModName, PROC pfnCurrent, PROC pfnNew)
	{
		...
		Toolhelp th(TH32CS_SNAPMODULE, GetCurrentProcessId());
...
//		__try {
			pImportDesc = (PIMAGE_IMPORT_DESCRIPTOR)os::imagehlp_dll::inst().ImageDirectoryEntryToData(hmodCaller, TRUE, IMAGE_DIRECTORY_ENTRY_IMPORT, &ulSize);
//		}
...
//		}
		...
		for (; pImportDesc->Name; pImportDesc++) {
			char* pszModName = (char*)((PBYTE)hmodCaller + pImportDesc->Name);
...
//		__try {
			pExportDir = (PIMAGE_EXPORT_DIRECTORY)os::imagehlp_dll::inst().ImageDirectoryEntryToData(hmod, TRUE, IMAGE_DIRECTORY_ENTRY_EXPORT, &ulSize);
//		}
//		__except(InvalidReadExceptionFilter(GetExceptionInformation ()))
//		{
//			// Nothing to do in here, thread continues to run normally
...
//		}
		...
		for (DWORD n = 0; n < pExportDir->NumberOfNames; n++) {
			...
			char* pszFuncName = (char*)((PBYTE)hmod + pdwNamesRvas[n]);
	...
	void CAPIHook::FixupNewlyLoadedModule(HMODULE hmod, DWORD dwFlags)
	{
		...
		if ((hmod != NULL)
		    ...
		    (hmod != ModuleFromAddress(reinterpret_cast<void*>(FixupNewlyLoadedModule))) && ((dwFlags & LOAD_LIBRARY_AS_DATAFILE) == 0) && ((dwFlags & LOAD_LIBRARY_AS_DATAFILE_EXCLUSIVE) == 0)
