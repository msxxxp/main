...
ssize_t sarastd::string_test()
{
	...
	Base::Console::printf("cc: '%s'\n", cc);

	Base::Console::printf("a (%Iu, %Iu): '%s'\n", a.size(), a.capacity(), a.c_str());
	Base::Console::printf("b (%Iu, %Iu): '%s'\n", b.size(), b.capacity(), b.c_str());

	a = b;
	Base::Console::printf("c (%Iu, %Iu): '%s'\n", c.size(), c.capacity(), c.c_str());
	Base::Console::printf("a (%Iu, %Iu): '%s'\n", a.size(), a.capacity(), a.c_str());
...
//		assert(*(str3.cend()-1) == '2');
	...
	Base::Console::printf("str2: '%s' size: %d capacity: %d\n", str2.c_str(), (int)str2.size(), (int)str2.capacity());
	assert(Cstr::compare(str2.c_str(), stdstr2.c_str()) == 0);
