...
///========================================================================================== Base64
	...
	namespace Crypt {
		...
		{
			return Memory::realloc(pbData, cbData);
		}

		bool DataBlob::reserve(size_t size)
		{
			if (size > cbData) {
				Memory::realloc(pbData, size);
				cbData = size;
				return true;
			}
			return false;
		...
		NameBlob::NameBlob(const ustring & in, DWORD enc)
		{
			...
			if (in.find(L'\"') != ustring::npos)
				Base::Flags::set(dwStrType, CERT_NAME_STR_NO_QUOTING_FLAG);
		...
		CertificateStore::CertificateStore(const ustring & in, StoreType type, DWORD flags) :
			m_hnd(nullptr), m_name(in)
		{
			switch (type) {
				case stMachine:
					Base::Flags::set(flags, (DWORD)CERT_SYSTEM_STORE_LOCAL_MACHINE);
					m_hnd = CheckHandle(::CertOpenStore(CERT_STORE_PROV_SYSTEM, 0, (HCRYPTPROV)nullptr, flags, m_name.c_str()));
					break;
				case stUser:
					Base::Flags::set(flags, (DWORD)CERT_SYSTEM_STORE_CURRENT_USER);
					m_hnd = CheckHandle(::CertOpenStore(CERT_STORE_PROV_SYSTEM, 0, (HCRYPTPROV)nullptr, flags, m_name.c_str()));
					break;
				case stMemory:
					Base::Flags::set(flags, (DWORD)CERT_STORE_CREATE_NEW_FLAG);
		...
		void Certificate::export_to_file(const ustring & path) const
		{
			...
			Fsys::File::write(path.c_str(), blob.pbData, blob.cbData);
		}
		...
		ustring Certificate::get_hash_string(PCCERT_CONTEXT pctx)
		{
			...
			return Base::to_str(buf.data(), buf.size());
		}
